...
const setupCustomImageUpload = (creator) => {
  ...
  creator.onUploadFile.add((survey, options) => {
    ...
    fetch(
      `${
        import.meta.env.VITE_API_URL || "https://ebmpk.a.pinggy.link/api"
      }/upload`,
      {
        method: "POST",
        body: formData,
      ...
      .then((result) => {
        if (result.fileUrl) {
          ...
          options.callback(
            "success",
            `${import.meta.env.VITE_API_URL || "https://ebmpk.a.pinggy.link"}${
              result.fileUrl
            }`
          );
        } else {
...
export const SurveyCreatorPage = () => {
  const [creator] = useState(() => {
    ...
    // Hide specific question types from toolbox
    // try {
    //   if (
    //     newCreator?.toolbox &&
    //     typeof newCreator.toolbox.removeItem === "function"
    //   ) {
    //     newCreator.toolbox.removeItem("multipletext");
    //   }
    // } catch {
    //   // Ignore toolbox API differences
    // }
  ...
  const saveSurvey = useCallback(async () => {
    try {
      ...
      const hasQuestions = surveyJSON.pages.some(
